...
if [ -d "plugin" ]; then
    ...
    if command -v zip >/dev/null 2>&1; then
        zip -r ../releases/onescript.plugin ./*
    elif command -v python3 >/dev/null 2>&1; then
        python3 -c "import zipfile, os; zipf = zipfile.ZipFile('../releases/onescript.plugin', 'w', zipfile.ZIP_DEFLATED); [zipf.write(os.path.join(root, f), os.path.relpath(os.path.join(root, f), '.')) for root, d, files in os.walk('.') for f in files]; zipf.close()"
    elif command -v python >/dev/null 2>&1; then
        python -c "import zipfile, os; zipf = zipfile.ZipFile('../releases/onescript.plugin', 'w', zipfile.ZIP_DEFLATED); [zipf.write(os.path.join(root, f), os.path.relpath(os.path.join(root, f), '.')) for root, d, files in os.walk('.') for f in files]; zipf.close()"
    else
