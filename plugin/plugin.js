(function (window, undefined) {
  ...
  window.Asc.plugin.init = function () {
    loadLogFile();
    log("S\\uper Editor v3 PDK initialized.", "success");
    loadSettings();
  ...
  chipSummarize.addEventListener("click", async () => {
    ...
    if (!apiKey) {
      log(
        "Error: Groq API Key is not configured. Add it in Settings.",
        "error",
      );
      tabSettings.click();
      return;
  ...
  executeBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      log(
        `Contacting Groq endpoint [api.groq.com/openai/v1] using model: ${modelSelect.value}...`,
        "info",
      );

      const aiResponse = await queryGroqAPI(
        apiKey,
        modelSelect.value,
        cachedDocData,
        prompt,
  ...
  function executeSequentialEdits(changes) {
    ...
    function applyNext() {
      ...
      if (actionName === "deleteParagraph") {
        log(
          `Executing Action: [Delete] Element #${change.targetIndex + 1}...`,
          "warning",
        );
      } else if (actionName === "createParagraph") {
        log(
          `Executing Action: [Create] New Paragraph after #${change.targetIndex + 1}...`,
          "success",
        );
      } else if (actionName === "pasteHTML") {
        log(
          `Executing Action: [Generative Typesetting] Elements starting at #${change.targetIndex + 1}...`,
          "success",
        );
        ...
        log(
          `Executing Action: [Formatting] Element #${change.targetIndex + 1}...`,
          "info",
        );
      }
      ...
      window.Asc.plugin.callCommand(
        function () {
                ...
                if (oRun) {
                  ...
                  if (formatState.highlight) {
                    try {
                      ...
                      else if (hl.indexOf("yellow") !== -1 || hl === "#ffff00")
                        oRun.SetHighlight("yellow");
                      else if (hl.indexOf("green") !== -1 || hl === "#00ff00")
              ...
              if (runCount > 0) {
                ...
                if (firstRun) {
                  ...
                  if (textPr) {
                    ...
                    try {
                      if (typeof firstRun.GetFontName === "function")
                        origFont = firstRun.GetFontName() || origFont;
                    } catch (e) {}
                    try {
                      if (typeof firstRun.GetFontSize === "function")
                        origSize = firstRun.GetFontSize() || origSize;
                    } catch (e) {}
                    try {
                      if (typeof firstRun.GetBold === "function")
                        origBold = firstRun.GetBold() || origBold;
                    } catch (e) {}
                    try {
                      if (typeof firstRun.GetItalic === "function")
                        origItalic = firstRun.GetItalic() || origItalic;
                    } catch (e) {}
                    try {
                      if (typeof firstRun.GetUnderline === "function")
                        origUnderline = !!firstRun.GetUnderline();
                    } catch (e) {}
                    try {
                      if (typeof firstRun.GetStrikeout === "function")
                        origStrikeout = !!firstRun.GetStrikeout();
                    } catch (e) {}
                    try {
          ...
          if (change.action === "deleteParagraph") {
            try {
              oDocument.RemoveElement(change.targetIndex);
            } catch (e) {}
            return "deleted";
          }
