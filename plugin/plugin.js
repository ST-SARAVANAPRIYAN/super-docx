(function (window, undefined) {
  ...
  const toggleGithubVisibility = document.getElementById(
    "toggle-github-visibility",
  );
  ...
  toggleGithubVisibility.addEventListener("click", () => {
    if (githubTokenInput.type === "password") {
      ...
      toggleGithubVisibility.innerText = "Hide";
    } else {
      githubTokenInput.type = "password";
      toggleGithubVisibility.innerText = "Show";
    }
  });
  ...
  const providerModels = {
    ...
    github: [
      ...
      { value: "gpt-4o-mini", text: "GPT-4o Mini (Fast & Smart)" },
      {
        ...
        text: "Llama 3.1 70B (Open Weight)",
      },
      {
        value: "meta-llama-3.1-405b-instruct",
        text: "Llama 3.1 405B (Max Power)",
      },
      {
        value: "mistral-large-2407",
        text: "Mistral Large (European Flagship)",
      },
      {
        value: "phi-3-medium-128k-instruct",
        text: "Phi-3 Medium (Lightweight)",
      },
    ],
  };
  ...
  function switchProvider(provider) {
    ...
    if (provider === "github") {
      ...
      modelSelect.innerHTML = providerModels.github
        .map((m) => `<option value="${m.value}">${m.text}</option>`)
        .join("");

      const savedGithubModel =
        localStorage.getItem("github_models_model") || "gpt-4o-mini";
      ...
      modelSelect.innerHTML = providerModels.groq
        .map((m) => `<option value="${m.value}">${m.text}</option>`)
        .join("");

      const savedGroqModel =
        localStorage.getItem("groq_copilot_model") || "llama-3.3-70b-versatile";
  ...
  function loadSettings() {
    ...
    const activeProvider =
      localStorage.getItem("super_editor_provider") || "groq";
    ...
    log(
      `Super Editor provider settings loaded securely. Active: ${activeProvider.toUpperCase()}`,
      "success",
    );
  }
  ...
  async function refreshDocStructureView() {
    ...
    try {
      ...
      log(
        `Compiled structural JSON successfully [Mode: ${parsed.mode}].`,
        "success",
      );
    } catch (err) {
  ...
  async function saveCheckpoint() {
    ...
    try {
      ...
      checkpoints.push(cp);
      log(
        `Successfully saved session Checkpoint #${checkpoints.length}!`,
        "success",
      );
      renderCheckpointsUI();
    } catch (err) {
  ...
  function renderCheckpointsUI() {
    ...
    checkpointsList
      .querySelectorAll(".checkpoint-restore-btn")
      .forEach((btn) => {
  ...
  chipSummarize.addEventListener("click", async () => {
    ...
    const hasToken =
      provider === "github"
        ? localStorage.getItem("github_models_token")
        : localStorage.getItem("groq_copilot_key");

    if (!hasToken) {
      log(
        `Error: ${provider === "github" ? "GitHub Token" : "Groq API Key"} is not configured. Add it in Settings.`,
        "error",
      );
      tabSettings.click();
      return;
    ...
    try {
      ...
      const parsed = JSON.parse(docJSON);
      log(
        `Summarizing active range [${parsed.mode === "selection" ? "Selection Only" : "Entire Document"}]...`,
        "info",
      );
      ...
      log(
        `Executive summary compiled successfully by ${provider === "github" ? "GitHub Models" : "Groq AI"}!`,
        "success",
      );
    } catch (err) {
  ...
  executeBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      const activeModel =
        provider === "github"
          ? localStorage.getItem("github_models_model")
          : localStorage.getItem("groq_copilot_model");

      log(
        `Successfully scanned ${cachedDocData.sections.length} sections containing ${totalElements} elements [Mode: ${cachedDocData.mode}].`,
        "success",
      );
      log(
        `Contacting ${provider === "github" ? "GitHub Models" : "Groq API"} using model: ${activeModel}...`,
        "info",
      );
      ...
      const aiResponse = await queryLLM(
        cachedDocData,
        prompt,
        cachedDocData.mode === "selection",
      );

      log(
        `Received secure API response from ${provider === "github" ? "GitHub Models" : "Groq"}.`,
        "success",
      );
      ...
      if (!proposedChanges || proposedChanges.length === 0) {
        log(
          "Analysis complete: No style or content changes suggested for this request.",
          "warning",
        );
        changesCard.style.display = "none";
      } else {
        log(
          `Successfully decoded ${proposedChanges.length} autonomous formatting instructions.`,
          "success",
        );
      ...
      if (aRuns && aRuns.length > 0) {
        for (var r = 0; r < aRuns.length; r++) {
          ...
          if (oRun && oRun.GetClassType() === "run") {
            ...
            try {
              if (oRun.GetFontName) rFontName = oRun.GetFontName() || rFontName;
            } catch (e) {}
            try {
              if (oRun.GetFontSize) rFontSize = oRun.GetFontSize() || rFontSize;
            } catch (e) {}
            try {
              if (oRun.GetBold) rBold = oRun.GetBold() || rBold;
            } catch (e) {}
            try {
              if (oRun.GetItalic) rItalic = oRun.GetItalic() || rItalic;
            } catch (e) {}
            try {
              if (oRun.GetColor) {
                var c = oRun.GetColor();
  ...
  function serializeActiveContent() {
    return new Promise((resolve, reject) => {
      const failTimeout = setTimeout(() => {
        reject(
          new Error(
            "Content scanning timed out. ONLYOFFICE sandbox did not respond.",
          ),
        );
      }, 5000);
      ...
      window.Asc.plugin.callCommand(
        function () {
          ...
          function extractRunStyle(oRun, oElement) {
            ...
            try {
              if (!rFontName && oRun.GetFontName)
                rFontName = oRun.GetFontName() || "";
            } catch (e) {}
            try {
              if (oRun.GetFontSize) rFontSize = oRun.GetFontSize() || 0;
            } catch (e) {}
            try {
              if (oRun.GetBold) rBold = oRun.GetBold();
            } catch (e) {}
            try {
              if (oRun.GetItalic) rItalic = oRun.GetItalic();
            } catch (e) {}
            try {
              if (oRun.GetUnderline) rUnderline = !!oRun.GetUnderline();
            } catch (e) {}
            try {
              if (oRun.GetStrikeout) rStrikeout = !!oRun.GetStrikeout();
            } catch (e) {}
            try {
              if (oRun.GetSmallCaps) rSmallCaps = !!oRun.GetSmallCaps();
            } catch (e) {}
            try {
              if (oRun.GetCaps) rCaps = !!oRun.GetCaps();
            } catch (e) {}
            try {
              if (oRun.GetSubscript) rSubscript = !!oRun.GetSubscript();
            } catch (e) {}
            try {
              if (oRun.GetSuperscript) rSuperscript = !!oRun.GetSuperscript();
            } catch (e) {}
            try {
              if (oRun.GetSpacing) rSpacing = oRun.GetSpacing() || 0;
            } catch (e) {}
            try {
              if (oRun.GetDoubleStrikeout)
                rDoubleStrikeout = !!oRun.GetDoubleStrikeout();
            } catch (e) {}
            try {
              if (oRun.GetColor) {
                var c = oRun.GetColor();
            ...
            try {
              if (oRun.GetTextPr) {
                ...
                if (tp) {
                  if (!rFontName && tp.GetFontFamily)
                    rFontName = tp.GetFontFamily() || "";
                  if (!rFontSize && tp.GetFontSize)
                    rFontSize = tp.GetFontSize() || 0;
                  if (rBold === null && tp.GetBold) rBold = tp.GetBold();
                  if (rItalic === null && tp.GetItalic)
                    rItalic = tp.GetItalic();
                  if (rUnderline === null && tp.GetUnderline)
                    rUnderline = !!tp.GetUnderline();
                  if (rStrikeout === null && tp.GetStrikeout)
                    rStrikeout = !!tp.GetStrikeout();
                  if (rSmallCaps === null && tp.GetSmallCaps)
                    rSmallCaps = !!tp.GetSmallCaps();
                  if (rCaps === null && tp.GetCaps) rCaps = !!tp.GetCaps();
                  if (rSubscript === null && tp.GetSubscript)
                    rSubscript = !!tp.GetSubscript();
                  if (rSuperscript === null && tp.GetSuperscript)
                    rSuperscript = !!tp.GetSuperscript();
                  if (rSpacing === 0 && tp.GetSpacing)
                    rSpacing = tp.GetSpacing() || 0;
                  if (rDoubleStrikeout === null && tp.GetDoubleStrikeout)
                    rDoubleStrikeout = !!tp.GetDoubleStrikeout();
              ...
              if (oElement && oElement.GetParaPr) {
                ...
                if (pPr && pPr.GetStyle) {
                  ...
                  if (style && style.GetTextPr) {
                    ...
                    if (stp) {
                      if (!rFontName && stp.GetFontFamily)
                        rFontName = stp.GetFontFamily() || "";
                      if (!rFontSize && stp.GetFontSize)
                        rFontSize = stp.GetFontSize() || 0;
                      if (rBold === null && stp.GetBold) rBold = stp.GetBold();
                      if (rItalic === null && stp.GetItalic)
                        rItalic = stp.GetItalic();
                      if (rUnderline === null && stp.GetUnderline)
                        rUnderline = !!stp.GetUnderline();
                      if (rStrikeout === null && stp.GetStrikeout)
                        rStrikeout = !!stp.GetStrikeout();
                      if (rSmallCaps === null && stp.GetSmallCaps)
                        rSmallCaps = !!stp.GetSmallCaps();
                      if (rCaps === null && stp.GetCaps)
                        rCaps = !!stp.GetCaps();
                      if (rSubscript === null && stp.GetSubscript)
                        rSubscript = !!stp.GetSubscript();
                      if (rSuperscript === null && stp.GetSuperscript)
                        rSuperscript = !!stp.GetSuperscript();
                      if (rSpacing === 0 && stp.GetSpacing)
                        rSpacing = stp.GetSpacing() || 0;
                      if (rDoubleStrikeout === null && stp.GetDoubleStrikeout)
                        rDoubleStrikeout = !!stp.GetDoubleStrikeout();
            ...
            try {
              if (selPara.GetParaPr) {
                ...
                if (pPr) {
                  ...
                  if (pPr.GetSpacingBefore)
                    oSpaceBefore = pPr.GetSpacingBefore() || 0;
                  if (pPr.GetSpacingAfter)
                    oSpaceAfter = pPr.GetSpacingAfter() || 0;
                  if (pPr.GetSpacingLineValue)
                    oLineSpacingTwips = pPr.GetSpacingLineValue() || 0;
                  if (pPr.GetSpacingLineRule)
                    oLineSpacingRule = pPr.GetSpacingLineRule() || "auto";

                  // Calculate line spacing multiplier
                  if (oLineSpacingRule === "auto" && oLineSpacingTwips > 0) {
                    oLineSpacing =
                      Math.round((oLineSpacingTwips / 240) * 100) / 100;
                  } else if (oLineSpacingTwips > 0) {
                    oLineSpacing =
                      Math.round((oLineSpacingTwips / 20) * 100) / 100; // in points
                  }
                  ...
                  if (pPr.GetIndFirstLine)
                    oIndFirstLine = pPr.GetIndFirstLine() || 0;
          ...
          function compressRunStyle(runStyle, paraStyle) {
            var compressed = {};
            if (runStyle.fontName !== paraStyle.fontName)
              compressed.fontName = runStyle.fontName;
            if (runStyle.fontSize !== paraStyle.fontSize)
              compressed.fontSize = runStyle.fontSize;
            if (runStyle.bold !== paraStyle.bold)
              compressed.bold = runStyle.bold;
            if (runStyle.italic !== paraStyle.italic)
              compressed.italic = runStyle.italic;
            if (runStyle.underline !== paraStyle.underline)
              compressed.underline = runStyle.underline;
            if (runStyle.strikeout !== paraStyle.strikeout)
              compressed.strikeout = runStyle.strikeout;
            if (runStyle.doubleStrikeout !== paraStyle.doubleStrikeout)
              compressed.doubleStrikeout = runStyle.doubleStrikeout;
            if (runStyle.smallCaps !== paraStyle.smallCaps)
              compressed.smallCaps = runStyle.smallCaps;
            if (runStyle.caps !== paraStyle.caps)
              compressed.caps = runStyle.caps;
            if (runStyle.subscript !== paraStyle.subscript)
              compressed.subscript = runStyle.subscript;
            if (runStyle.superscript !== paraStyle.superscript)
              compressed.superscript = runStyle.superscript;
            if (
              runStyle.characterSpacing !== paraStyle.characterSpacing &&
              runStyle.characterSpacing !== 0
            )
              compressed.characterSpacing = runStyle.characterSpacing;
            if (
              runStyle.color !== paraStyle.color &&
              runStyle.color !== "#000000"
            )
              compressed.color = runStyle.color;
            if (runStyle.highlight && runStyle.highlight !== "none")
              compressed.highlight = runStyle.highlight;
          ...
          for (var h = 0; h < elementsCount; h++) {
            try {
              ...
              if (oElem && oElem.GetClassType() === "paragraph") {
                ...
                if (
                  paraStyle.styleName &&
                  paraStyle.styleName.toLowerCase().indexOf("heading") !== -1
                ) {
                  isHeading = true;
                } else if (
                  (paraStyle.fontSize * 2 >= 28 ||
                    (paraStyle.bold && paraStyle.fontSize * 2 >= 24)) &&
                  cleanText.length > 0 &&
                  ...
                  /^(Chapter\s+\d+|\d+(\.\d+)*\s+[A-Za-z])/i.test(cleanText) &&
                ...
                if (isHeading) {
                  documentHeadings.push({
                    ...
                    styleName:
                      paraStyle.styleName ||
                      "Heading (Size " + paraStyle.fontSize + "pt)",
                  });
                }
              }
            } catch (eOutline) {}
          ...
          try {
            ...
            if (firstSection) {
              pageSettings = {};
              try {
                if (firstSection.GetPageWidth)
                  pageSettings.pageWidth = firstSection.GetPageWidth();
              } catch (e) {}
              try {
                if (firstSection.GetPageHeight)
                  pageSettings.pageHeight = firstSection.GetPageHeight();
              } catch (e) {}
              try {
                if (firstSection.GetPageOrientation)
                  pageSettings.pageOrientation =
                    firstSection.GetPageOrientation();
              } catch (e) {}
              try {
                if (firstSection.GetMarginLeft)
                  pageSettings.marginLeft = firstSection.GetMarginLeft();
              } catch (e) {}
              try {
                if (firstSection.GetMarginRight)
                  pageSettings.marginRight = firstSection.GetMarginRight();
              } catch (e) {}
              try {
                if (firstSection.GetMarginTop)
                  pageSettings.marginTop = firstSection.GetMarginTop();
              } catch (e) {}
              try {
                if (firstSection.GetMarginBottom)
                  pageSettings.marginBottom = firstSection.GetMarginBottom();
              } catch (e) {}
            }
          } catch (ePage) {}
            ...
            for (var i = 0; i < selectedParagraphs.length; i++) {
              ...
              if (absoluteIndex === -1) {
                ...
                for (var j = 0; j < elementsCount; j++) {
                  ...
                  if (
                    docElem &&
                    docElem.GetClassType() === "paragraph" &&
                    docElem.GetText() === selText
                  ) {
                    absoluteIndex = j;
                    break;
                  }
                ...
                for (var r = 0; r < runCount; r++) {
                  ...
                  if (oRun && oRun.GetClassType() === "run") {
                    ...
                    if (intersectStart < intersectEnd) {
                      var slicedText = rText.substring(
                ...
                try {
                  ...
                  if (oElem && oElem.GetClassType() === "paragraph") {
                    precedingContext.push({
                      index: p,
                      text: oElem.GetText() || "",
                      style: compressParagraphStyle(
                        extractParagraphStyle(oElem),
                      ),
                    });
                  }
                } catch (ePre) {}
            ...
            for (var i = 0; i < limit; i++) {
              try {
                ...
                if (type === "paragraph") {
                  ...
                  if (
                    /^(Chapter\s+\d+|\d+(\.\d+)*\s+[A-Za-z])/i.test(
                      cleanText,
                    ) &&
                    cleanText.length < 100
                  ) {
                    ...
                    for (var r = 0; r < rowLimit; r++) {
                      ...
                      if (oRow) {
                        ...
                        for (var c = 0; c < cellLimit; c++) {
                          ...
                          var cellText = "";
                          if (oCell) {
                            var cellParagraphs = oCell
                              .GetContent()
                              .GetAllParagraphs();
                            var pTexts = [];
                            for (
                              var p = 0;
                              p < Math.min(cellParagraphs.length, 5);
                              p++
                            ) {
          ...
          return JSON.stringify({
            ...
            metadata: {
              ...
              selectedElementsCount: isSelection
                ? selectedParagraphs.length
                : elementsCount,
              selectionRange: isSelection
                ? { start: firstAbsIndex, end: lastAbsIndex }
                : null,
        ...
        function (result) {
          ...
          if (result) {
            ...
            reject(
              new Error("Unable to read active document/selection contents."),
            );
          }
        },
      );
  ...
  async function queryActiveLLM(
    ...
    if (currentProvider === "github") {
      ...
      if (!token) {
        throw new Error(
          "GitHub Token (PAT) is not configured. Add it in Settings.",
        );
      }
...
"${prompt}"`;
  ...
  function parseAIResponse(responseStr) {
    try {
      ...
      throw new Error(
        "Could not parse a valid JSON action script from the AI's response.",
      );
    }
  }
  ...
  function renderPreview(changes) {
    ...
    changes.forEach((change) => {
      ...
      if (change.action === "createParagraph") {
        ...
        actionBadge =
          '<span class="badge-action action-generate" style="background: rgba(139, 92, 246, 0.15) !important; color: #a78bfa !important; border: 1px solid rgba(139, 92, 246, 0.3) !important;">Generate</span>';
      } else {
      ...
      if (props.bold !== undefined)
        formatStr += props.bold ? "<b>Bold</b>; " : "Regular; ";
      if (props.italic !== undefined)
        formatStr += props.italic ? "<i>Italic</i>; " : "No Italic; ";
      if (props.underline !== undefined)
        formatStr += props.underline ? "<u>Underline</u>; " : "No Underline; ";
      if (props.strikeout !== undefined)
        formatStr += props.strikeout
          ? "<strike>Strike</strike>; "
          : "No Strike; ";
      if (props.alignment) formatStr += `Align: <b>${props.alignment}</b>; `;
      if (props.color)
        formatStr += `Color: <span style="color: ${props.color}; font-weight: bold;">${props.color}</span>; `;
      if (props.highlight)
        formatStr += `Highlight: <span style="background-color: ${props.highlight}; color: #000; font-weight: bold; padding: 1px 4px; border-radius: 3px;">${props.highlight}</span>; `;

      const originalText = original
        ? original.text || `[Table Element at index #${change.targetIndex}]`
        : "";
  ...
  function executeSequentialEdits(changes) {
    ...
    function applyNext() {
      ...
      if (actionName === "deleteParagraph") {
        log(
          `Executing Action: [Delete] Element #${targetIndex + 1} (actual target #${actualTargetIndex + 1})...`,
          "warning",
        );
        window.Asc.plugin.callCommand(
          function () {
            ...
            try {
              oDocument.RemoveElement(actualTargetIndex);
            } catch (e) {}
          ...
          function (res) {
            ...
            indexOffset += delta;
            log(
              `Dynamic Index Offset adjusted by ${delta}. Total offset: ${indexOffset}`,
              "info",
            );
            i++;
        ...
        log(
          `Executing Action: [Create] New Paragraph after #${targetIndex + 1} (actual target #${actualTargetIndex + 1})...`,
          "success",
        );
        window.Asc.plugin.callCommand(
          function () {
                ...
                if (runCount > 0) {
                  ...
                  if (firstRun) {
                    ...
                    if (textPr) {
                      ...
                      try {
                        if (typeof textPr.GetFontSize === "function")
                          origSize = textPr.GetFontSize() || origSize;
                      } catch (e) {}
                      try {
                        if (typeof textPr.GetBold === "function")
                          origBold = textPr.GetBold() || origBold;
                      } catch (e) {}
                      try {
                        if (typeof textPr.GetItalic === "function")
                          origItalic = textPr.GetItalic() || origItalic;
                      } catch (e) {}
                      try {
                        if (typeof textPr.GetUnderline === "function")
                          origUnderline = !!textPr.GetUnderline();
                      } catch (e) {}
                      try {
                        if (typeof textPr.GetStrikeout === "function")
                          origStrikeout = !!textPr.GetStrikeout();
                      } catch (e) {}
                      try {
                        if (typeof textPr.GetColor === "function") {
                          var c = textPr.GetColor();
                          if (c && typeof c.GetHex === "function")
                            origColorHex = c.GetHex() || origColorHex;
                        }
                      } catch (e) {}
                      try {
                        if (typeof textPr.GetHighlight === "function") {
                          ...
                          if (hl) {
                            ...
                            else if (typeof hl.GetHex === "function")
                              origHighlight = hl.GetHex() || origHighlight;
                          }
                        }
                      } catch (e) {}
                    }
            ...
            function parseAndApplyTextWithTags(
              oPar,
              htmlStr,
              defFont,
              defSize,
              ...
              var formatState = {
                ...
                bold:
                  pProps.bold !== undefined
                    ? pProps.bold
                    : defBold !== undefined
                      ? defBold
                      : false,
                italic:
                  pProps.italic !== undefined
                    ? pProps.italic
                    : defItalic !== undefined
                      ? defItalic
                      : false,
                underline:
                  pProps.underline !== undefined
                    ? pProps.underline
                    : defUnderline !== undefined
                      ? defUnderline
                      : false,
                strikeout:
                  pProps.strikeout !== undefined
                    ? pProps.strikeout
                    : defStrikeout !== undefined
                      ? defStrikeout
                      : false,
                doubleStrikeout:
                  pProps.doubleStrikeout !== undefined
                    ? pProps.doubleStrikeout
                    : false,
                smallCaps:
                  pProps.smallCaps !== undefined ? pProps.smallCaps : false,
                caps: pProps.caps !== undefined ? pProps.caps : false,
                subscript:
                  pProps.subscript !== undefined ? pProps.subscript : false,
                superscript:
                  pProps.superscript !== undefined ? pProps.superscript : false,
                characterSpacing:
                  pProps.characterSpacing !== undefined
                    ? pProps.characterSpacing
                    : 0,
              ...
              for (var idx = 0; idx < parts.length; idx++) {
                ...
                if (
                  ...
                  if (tagLower.indexOf("</") === 0) {
                    ...
                    if (
                      tagLower.indexOf("<b") === 0 ||
                      tagLower.indexOf("<strong") === 0
                    ) {
                      newState.bold = true;
                    } else if (
                      tagLower.indexOf("<i") === 0 ||
                      tagLower.indexOf("<em") === 0
                    ) {
                      newState.italic = true;
                    } else if (tagLower.indexOf("<u") === 0) {
                      newState.underline = true;
                    } else if (
                      tagLower.indexOf("<strike") === 0 ||
                      tagLower.indexOf("<del") === 0 ||
                      tagLower.indexOf("<s") === 0
                    ) {
                      ...
                      if (matchFace && matchFace[1])
                        newState.fontName = matchFace[1];
                      var matchSize = part.match(/size=["']([^"']+)["']/i);
                      if (matchSize && matchSize[1])
                        newState.fontSize = parseFloat(matchSize[1]);
                        ...
                        for (var d = 0; d < declarations.length; d++) {
                          ...
                          if (pSplit.length >= 2) {
                            ...
                            if (propName === "font-weight") {
                              ...
                              if (propVal === "italic" || propVal === "oblique")
                                newState.italic = true;
                              else if (propVal === "normal")
                                newState.italic = false;
                            } else if (propName === "text-decoration") {
                              if (propVal.indexOf("underline") !== -1)
                                newState.underline = true;
                              if (propVal.indexOf("line-through") !== -1)
                                newState.strikeout = true;
                              if (
                                propVal.indexOf("double-line-through") !== -1 ||
                                propVal.indexOf("double") !== -1
                              )
                                newState.doubleStrikeout = true;
                              ...
                              var numVal = parseFloat(propVal);
                              if (propVal.indexOf("pt") !== -1) {
                                newState.characterSpacing = Math.round(
                                  numVal * 20,
                                ); // 1pt letter-spacing = 20 dxa character spacing
                              } else if (propVal.indexOf("px") !== -1) {
                                newState.characterSpacing = Math.round(
                                  numVal * 15,
                                );
                              ...
                              else if (propVal === "lowercase")
                                newState.caps = false;
                            } else if (propName === "font-variant") {
                              if (propVal === "small-caps")
                                newState.smallCaps = true;
                  ...
                  if (oRun) {
                    ...
                    try {
                      oRun.SetItalic(!!formatState.italic);
                    } catch (e) {}
                    try {
                      oRun.SetUnderline(!!formatState.underline);
                    } catch (e) {}
                    try {
                      oRun.SetStrikeout(!!formatState.strikeout);
                    } catch (e) {}
                    try {
                      oRun.SetDoubleStrikeout(!!formatState.doubleStrikeout);
                    } catch (e) {}
                    try {
                      oRun.SetSmallCaps(!!formatState.smallCaps);
                    } catch (e) {}
                    try {
                      oRun.SetCaps(!!formatState.caps);
                    } catch (e) {}
                    try {
                      oRun.SetSubscript(!!formatState.subscript);
                    } catch (e) {}
                    try {
                      oRun.SetSuperscript(!!formatState.superscript);
                    } catch (e) {}
                    try {
                      if (formatState.characterSpacing)
                        oRun.SetSpacing(formatState.characterSpacing);
                    } catch (e) {}

                    if (formatState.highlight) {
                      try {
                        var hl = formatState.highlight.toLowerCase().trim();
                        if (hl === "none" || hl === "null" || hl === "default")
                          oRun.SetHighlight("none");
                        else if (
                          hl.indexOf("yellow") !== -1 ||
                          hl === "#ffff00"
                        )
                        ...
                        else if (hl.indexOf("blue") !== -1 || hl === "#0000ff")
                          oRun.SetHighlight("blue");
                        else if (
                          hl.indexOf("cyan") !== -1 ||
                          hl.indexOf("aqua") !== -1 ||
                          hl === "#00ffff"
                        )
                          oRun.SetHighlight("cyan");
                        else if (hl.indexOf("red") !== -1 || hl === "#ff0000")
                          oRun.SetHighlight("red");
                        else if (
                          hl.indexOf("magenta") !== -1 ||
                          hl.indexOf("pink") !== -1 ||
                          hl === "#ff00ff"
                        )
                          oRun.SetHighlight("magenta");
                        else if (
                          hl.indexOf("gray") !== -1 ||
                          hl.indexOf("grey") !== -1 ||
                          hl === "#808080"
                    ...
                    if (formatState.color) {
                      try {
                        var hex = String(formatState.color)
                          .replace("#", "")
                          .trim();
                        if (hex.length === 6) {
                          ...
                          if (namedColors[hex.toLowerCase()]) {
                            ...
                            oRun.SetColor(
                              Api.CreateColorFromRGB(rgb[0], rgb[1], rgb[2]),
                            );
                          }
                        }
                      } catch (eColorOuter) {}
        ...
        log(
          `Executing Action: [Generative Typesetting] Elements starting at #${targetIndex + 1} (actual target #${actualTargetIndex + 1})...`,
          "success",
        );
            ...
            window.Asc.plugin.executeMethod(
              ...
              function () {
                ...
                window.Asc.plugin.callCommand(
                  ...
                  function (countAfter) {
                    var delta =
                      countAfter !== undefined && countBefore !== undefined
                        ? countAfter - countBefore
                        : 0;
                    indexOffset += delta;
                    log(
                      `Dynamic Index Offset adjusted by ${delta} after PasteHtml. Total offset: ${indexOffset}`,
                      "info",
                    );
                    i++;
        ...
        log(
          `Executing Action: [Formatting] Element #${targetIndex + 1} (actual target #${actualTargetIndex + 1})...`,
          "info",
        );
        window.Asc.plugin.callCommand(
          function () {
            ...
            if (oParagraph) {
                  ...
                  for (var r = 0; r < runCount; r++) {
                    ...
                    if (oRun && oRun.GetClassType() === "run") {
                      ...
                      if (oProps.italic !== undefined) {
                        try {
                          oRun.SetItalic(!!oProps.italic);
                        } catch (e) {}
                      }
                      if (oProps.underline !== undefined) {
                        try {
                          oRun.SetUnderline(!!oProps.underline);
                        } catch (e) {}
                      }
                      if (oProps.strikeout !== undefined) {
                        try {
                          oRun.SetStrikeout(!!oProps.strikeout);
                        } catch (e) {}
                      }
                      if (oProps.doubleStrikeout !== undefined) {
                        try {
                          oRun.SetDoubleStrikeout(!!oProps.doubleStrikeout);
                        } catch (e) {}
                      }
                      if (oProps.smallCaps !== undefined) {
                        try {
                          oRun.SetSmallCaps(!!oProps.smallCaps);
                        } catch (e) {}
                      }
                      if (oProps.caps !== undefined) {
                        try {
                          oRun.SetCaps(!!oProps.caps);
                        } catch (e) {}
                      }
                      if (oProps.subscript !== undefined) {
                        try {
                          oRun.SetSubscript(!!oProps.subscript);
                        } catch (e) {}
                      }
                      if (oProps.superscript !== undefined) {
                        try {
                          oRun.SetSuperscript(!!oProps.superscript);
                        } catch (e) {}
                      }
                      if (oProps.characterSpacing !== undefined) {
                        try {
                          oRun.SetSpacing(oProps.characterSpacing);
                        } catch (e) {}
                      }
                      ...
                      if (oProps.color !== undefined) {
                        try {
                          ...
                          if (hex.length === 6) {
                            ...
                            try {
                              oRun.SetColor(
                                Api.CreateColorFromRGB(red, green, blue),
                              );
                            } catch (eColor) {
                              try {
                                oRun.SetColor(red, green, blue);
                              } catch (errHex) {}
                            }
                          } else {
                            var namedColors = {
              ...
              try {
                if (oProps.alignment) {
                  ...
                  if (typeof oParagraph.SetJc === "function")
                    oParagraph.SetJc(jc);
                }
              } catch (e) {}
              try {
                if (
                  oProps.spacingAfter !== undefined &&
                  oParagraph.SetSpacingAfter
                )
                  oParagraph.SetSpacingAfter(oProps.spacingAfter);
              } catch (e) {}
              try {
                if (
                  oProps.spacingBefore !== undefined &&
                  oParagraph.SetSpacingBefore
                )
                  oParagraph.SetSpacingBefore(oProps.spacingBefore);
              } catch (e) {}
              try {
                if (
                  oProps.lineSpacing !== undefined &&
                  oParagraph.SetSpacingLine
                ) {
                  var rule = oProps.lineSpacingRule || "auto";
                  var val =
                    oProps.lineSpacingTwips ||
                    Math.round(oProps.lineSpacing * 240);
                  oParagraph.SetSpacingLine(val, rule);
                }
              } catch (e) {}
              try {
                if (oProps.indLeft !== undefined && oParagraph.SetIndLeft)
                  oParagraph.SetIndLeft(oProps.indLeft);
              } catch (e) {}
              try {
                if (oProps.indRight !== undefined && oParagraph.SetIndRight)
                  oParagraph.SetIndRight(oProps.indRight);
              } catch (e) {}
              try {
                if (
                  oProps.indFirstLine !== undefined &&
                  oParagraph.SetIndFirstLine
                )
                  oParagraph.SetIndFirstLine(oProps.indFirstLine);
              } catch (e) {}
              try {
                if (oProps.shading !== undefined && oParagraph.SetShd) {
                  ...
                  if (hex.length === 6) {
                    ...
                    var b = parseInt(hex.substring(4, 6), 16);
                    try {
                      oParagraph.SetShd(r, g, b);
                    } catch (eShd) {
                      try {
                        oParagraph.SetShd(Api.CreateColorFromRGB(r, g, b));
                      } catch (errShd) {}
                    }
                  }
                }
