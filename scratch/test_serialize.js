  ...
  for (let i = 0; i < documentElements.length; i++) {
    ...
    for (let j = 0; j < selectedLines.length; j++) {
      ...
      if (cleanSel.length > 0) {
        if (
          cleanPText === cleanSel ||
          cleanPText.indexOf(cleanSel) !== -1 ||
          cleanSel.indexOf(cleanPText) !== -1
        ) {
...
const mockDoc = [
  ...
  new MockParagraph("We affirm that the Project report is original.\n", [
    new MockRun(
      "We affirm that the Project report is original.\n",
      "Calibri",
      22,
      false,
      false,
  ...
  new MockParagraph(
    "FarmSentinel - Wildlife Threat Detection and Drone Guidance System for Crop Protection\n",
    [
      new MockRun("FarmSentinel - ", "Calibri", 22, true, false, "#000000"), // nested bold phrase!
      new MockRun(
        "Wildlife Threat Detection and Drone Guidance System for Crop Protection\n",
        "Calibri",
        22,
        false,
        false,
  ...
  new MockParagraph("This is unrelated, but contains word of wisdom.\n", [
    new MockRun(
      "This is unrelated, but contains word of wisdom.\n",
      "Calibri",
      22,
      false,
      false,
...
const matchTitle = testSelectionMatching(
  "FarmSentinel - Wildlife Threat Detection and Drone Guidance System for Crop Protection",
  mockDoc,
);
